...
BEGIN
  ...
  FOR v_i IN 0..9 LOOP
    ...
    -- Morning pass: current exchange limits before the regular session.
    v_min := 15 + (v_i * 3);
    ...
    -- Closing pass: refresh every page after 15:30 IST so next scans use the
    -- latest published upper/lower circuit limits for that trading day.
    v_min := 30 + (v_i * 3);
    PERFORM cron.schedule(
      format('refresh-price-bands-groww-close-%s', v_i),
