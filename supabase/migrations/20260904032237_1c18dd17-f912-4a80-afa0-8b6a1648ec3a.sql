-- lovable-cron-fallback-reviewed: 144 runs/day; pre-existing 10-minute reminder delivery job preserved unchanged; the new job added here is weekly.
...
BEGIN
  ...
  PERFORM cron.unschedule(jobid) FROM cron.job
   WHERE jobname IN ('refresh-snapshot','refresh-snapshot-evening','snapshot-breadth','refresh-vol-maxes',
                     ...
                     'evaluate-alerts-0500','evaluate-alerts-0800','deliver-reminders',
                     'cleanup-band-changes-weekly')
  ...
  -- Personal reminders: checked every 10 minutes so the chosen time is honoured.
  PERFORM cron.schedule('deliver-reminders', '*/10 * * * *',
    format($job$ SELECT net.http_post(
      url:='https://kch-tv.lovable.app/api/public/cron/deliver-reminders',
      headers:=jsonb_build_object('Content-Type','application/json','x-cron-secret',%L),
      body:='{}'::jsonb); $job$, _secret));
  v_installed := v_installed + 1;

  -- Friday 19:00 IST (13:30 UTC): trim circuit band history to the latest 5 trading days.
  PERFORM cron.schedule('cleanup-band-changes-weekly', '30 13 * * 5',
    format($job$ SELECT net.http_post(
      url:='https://kch-tv.lovable.app/api/public/cron/cleanup-band-changes',
      headers:=jsonb_build_object('Content-Type','application/json','x-cron-secret',%L),
      body:='{}'::jsonb); $job$, _secret));
  v_installed := v_installed + 1;
